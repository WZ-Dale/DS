...
int main() {
	...
	n1->random = n3;	// 指向后边的结点
	n2->random = n1;	// 指向前边的结点
	n3->random = n3;	// 指向自己
	n4->random = NULL;	// 指向空结点
...
//复制该链表:1.先复制next和value: 让新节点跟在老结点后面	2.再复制random	3.再把链表拆成两个
typedef struct RNode{
	int value;
	struct RNode *random;
	struct RNode *next;
...
typedef struct Node {
	int value;
	struct Node *next;//第一个指向
	struct Node *prev;//第二个指向
}Node;
